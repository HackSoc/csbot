...
async def test_term_dates(bot_helper, time_machine):
    ...
    bot_helper.assert_sent(bot_helper.match_line.endswith("error: no term dates (see termdates.set)"))
    ...
    bot_helper.assert_sent([
        bot_helper.match_line.endswith("Aut 2021-09-27 -- 2021-12-03, "
...
async def test_week_command(bot_helper, time_machine):
    ...
    bot_helper.assert_sent(bot_helper.match_line.endswith("Aut 3: 2021-10-11"))
    await asyncio.wait(bot_helper.receive(say("!week spr 10")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("Spr 10: 2022-03-14"))
    await asyncio.wait(bot_helper.receive(say("!week sum 4")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("Sum 4: 2022-05-09"))
    ...
    await asyncio.wait(bot_helper.receive(say("!week")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("9th week before Aut (starts 2021-09-27)"))
    ...
    await asyncio.wait(bot_helper.receive(say("!week")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("Aut 4: 2021-10-18"))
    # `!week n` should give the start of the Nth week in the Autumn term
    await asyncio.wait(bot_helper.receive(say("!week 3")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("Aut 3: 2021-10-11"))
    ...
    await asyncio.wait(bot_helper.receive(say("!week")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("4th week before Spr (starts 2022-01-10)"))
    # `!week n` should give the start of the Nth week in the Spring term
    await asyncio.wait(bot_helper.receive(say("!week 3")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("Spr 3: 2022-01-24"))
    ...
    await asyncio.wait(bot_helper.receive(say("!week")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("2nd week before Sum (starts 2022-04-19)"))
    # `!week n` should give the start of the Nth week in the Summer term
    await asyncio.wait(bot_helper.receive(say("!week 3")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("Sum 3: 2022-05-02"))

    # Time travel to during the Summer term, week 7
    time_machine.move_to(datetime.datetime(2022, 5, 31, 12, 0))
    # `!week` should give "Sum 7: ..."
    await asyncio.wait(bot_helper.receive(say("!week")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("Sum 7: 2022-05-30"))
    # `!week n` should give the start of the Nth week in the Summer term
    await asyncio.wait(bot_helper.receive(say("!week 3")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("Sum 3: 2022-05-02"))

    # Time travel to after the Summer term
    time_machine.move_to(datetime.datetime(2022, 8, 15, 12, 0))
    # `!week` should error because no current or next term
    await asyncio.wait(bot_helper.receive(say("!week")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("error: no term dates (see termdates.set)"))
    # `!week n` should error because no current term
    await asyncio.wait(bot_helper.receive(say("!week 3")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("error: no term dates (see termdates.set)"))

    # Edge case: time travel to the Monday of the first week of a term that starts on Tuesday
    time_machine.move_to(datetime.datetime(2022, 4, 18))
    await asyncio.wait(bot_helper.receive(say("!week")))
    bot_helper.assert_sent(bot_helper.match_line.endswith("Sum 1: 2022-04-19"))
