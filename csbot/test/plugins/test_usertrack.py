...
class TestUserTrackPlugin(BotTestCase):
    ...
    def test_join_part(self):
        ...
        self.protocol_.lineReceived(":Nick!~user@hostname JOIN #channel accountname :Other Info")
        self._assert_channels('Nick', {'#channel'})

        self._assert_channels('Other', set())
        self.protocol_.lineReceived(":Other!~user@hostname JOIN #channel accountname :Other Info")
        self._assert_channels('Other', {'#channel'})

        self.protocol_.lineReceived(":Other!~user@hostname JOIN #other accountname :Other Info")
        self._assert_channels('Other', {'#channel', '#other'})
        self._assert_channels('Nick', {'#channel'})

        self.protocol_.lineReceived(":Other!~user@hostname PART #channel")
        self._assert_channels('Other', {'#other'})

    def test_join_names(self):
        raise unittest.SkipTest('NAMES messages too dependent on stateful IRCClient operation, '
                                'too difficult to test')
        self._assert_channels('Nick', set())
        self._assert_channels('Other', set())
        # Initialise "PREFIX" feature so "NAMES" support works correctly
        self.protocol_.lineReceived(":server 005 self PREFIX=(ov)@+ :are supported by this server")
        self.protocol_.lineReceived(":server 353 self @ #channel :Nick Other")
        self.protocol_.lineReceived(":server 366 self #channel :End of /NAMES list.")
        self._assert_channels('Nick', {'#channel'})
        self._assert_channels('Other', {'#channel'})

    def test_quit_channels(self):
        self.protocol_.lineReceived(":Nick!~user@hostname JOIN #channel * :Other Info")
        self._assert_channels('Nick', {'#channel'})
        self.protocol_.lineReceived(":Nick!~user@hostname QUIT :Quit message")
        self._assert_channels('Nick', set())
    ...
    def test_nick_changed(self):
        ...
        self.protocol_.lineReceived(':Nick!~user@hostname NICK :Other')
        self._assert_channels('Nick', set())
        self._assert_channels('Other', {'#channel'})
    ...
    def test_account_forgotten_on_lost_visibility(self):
        ...
        self.protocol_.lineReceived(":Nick!~user@hostname JOIN #channel accountname :Other Info")
        self._assert_account('Nick', 'accountname')
        self.protocol_.lineReceived(":Nick!~user@hostname JOIN #other accountname :Other Info")
        ...
        self.protocol_.lineReceived(":Nick!~user@hostname PART #channel")
        self._assert_account('Nick', 'accountname')
        self._assert_channels('Nick', {'#other'})
        # User leaves last remaining channel, account should be forgotten
        self.protocol_.lineReceived(":Nick!~user@hostname PART #other")
        self._assert_account('Nick', None)
    ...
    def test_account_notify(self):
        ...
        self._assert_account('Nick', None)
        self._assert_channels('Nick', {'#channel'})
        self.protocol_.lineReceived(":Nick!~user@hostname ACCOUNT accountname")
        self._assert_account('Nick', 'accountname')
        self._assert_channels('Nick', {'#channel'})
        self.protocol_.lineReceived(":Nick!~user@hostname ACCOUNT *")
        self._assert_account('Nick', None)
        self._assert_channels('Nick', {'#channel'})
