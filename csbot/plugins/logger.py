...
class Logger(Plugin):
    ...
    def joined(self, event):
        self.pretty_log.info(u'[Joined {0}]'.format(event['channel']))

    @Plugin.hook('core.self.left')
    def left(self, event):
        self.pretty_log.info(u'[Left {0}]'.format(event['channel']))
    ...
    def quit(self, event):
        self.pretty_log.info(u'{user} has quit'.format(user=event['user']))
    ...
    def command(self, event):
        self.pretty_log.info(
            u'Command {command} fired by {nick} in channel {channel}'.format(
