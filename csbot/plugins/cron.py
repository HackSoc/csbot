...
class Cron(Plugin):
    ...
    def setup(self):
        ...
        self.tasks.remove({'name': 'cron.hourly'})
        self.tasks.remove({'name': 'cron.daily'})
        self.tasks.remove({'name': 'cron.weekly'})
    ...
    def setup_regular(self, now, name, tdelta):
        ...
        self.schedule(name, timedelta(seconds=tdelta), func, repeat=True)
